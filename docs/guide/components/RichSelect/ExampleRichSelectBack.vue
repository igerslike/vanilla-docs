<template>
  <div class="h-[1000px]">
    ...
    <div class="flex items-center justify-center mx-auto text-center mt-2">
      <pre>{{ JSON.stringify(optionOneValue) }}</pre>
    </div>
    ...
    >
      <template #label="{ option, optionRaw, className, isSelected }">
        <VanillaRichSelectOptionWithIndicators
          :name="option.raw.text"
          :status="option.raw.status"
...
const optionOneValue = ref('Option 1');
